       ...
       <v-navigation-drawer
        ...
        <v-container>
            ...
            <div class="d-flex flex-row mb-2">
                ...
                <v-spacer></v-spacer>
                <div style="cursor:pointer " @click="clearFilter()" class="mt-2 caption blue--text">
                    Clear Filter
                </div>
        ...
        <div v-if="toggle_exclusive == 0">
            <v-text-field
                        ...
                        class="mt-3"
                        v-model="searchaxie"
                        placeholder="Search Axie ID"
                        prepend-inner-icon="mdi-magnify"
                        v-on:keyup.enter="search_axie()"
            ></v-text-field>
            <v-card
            color="transparent"
            outlined>
                <h5 class="overline">Class</h5>
            ...
            <v-card class="mt-3" color="transparent" outlined>
                <h5 class="overline">Stage</h5>
                <div class="d-flex flex-row">
                    <div class="mr-5">
                        <v-checkbox
            ...
            class="mt-3"
            >
                <h5 class="overline">Breed Count</h5>
                <v-range-slider
           ...
           <v-combobox
            ...
            >
                <template slot='item' slot-scope='{ item }'>
                    <v-list-item @click="append_selected_search_parts(item.name)">
                        <v-list-item-icon>
                            <span :inner-html.prop="item.svg"></span>
                        </v-list-item-icon>
                        <v-list-item-content>
                ...
                <v-list rounded v-if="selected_parts.length != 0">
                <v-list-item-group>
                    <v-list-item
                    v-for="(item, i) in selected_parts"
                    :key="i"
                    @click="preppend_selected_search_parts(item.name)"
                    >
                    <v-list-item-icon>
                        <span :inner-html.prop="item.svg"></span>
                    </v-list-item-icon>
            ...
            outlined>
                ...
                <v-list-item-group>
                    ...
                    :key="i"
                    @click="append_selected_search_parts(item.name)"
                    >
                    <v-list-item-icon>
                        <span :inner-html.prop="item.svg"></span>
                    </v-list-item-icon>
        ...
        <div v-if="toggle_exclusive == 2">
            ...
            class="mt-10"
            >
                <small class="overline grey--text">
                ...
                small
                text
                @click="health_stat= [27,61]"
                >Reset</v-btn>
                <v-range-slider
                :hint="`${health_stat[0]}` +' max of ' + `${health_stat[1]}` "
                v-model="health_stat"
                :max="max_stat"
                :min="min_stat"
                persistent-hint
                ...
                <template v-slot:message>
                    <span class="float-left">{{health_stat[0]}}</span>
                    <span class="float-right">{{health_stat[1]}}</span>
                </template>
            ...
            >
                ...
                small
                text
                @click="speed_stat= [27,61]"
                >Reset</v-btn>
                <v-range-slider
                :hint="`${speed_stat[0]}` +' max of ' + `${speed_stat[1]}` "
                v-model="speed_stat"
                :max="max_stat"
                :min="min_stat"
                persistent-hint
                ...
                <template v-slot:message>
                    <span class="float-left">{{speed_stat[0]}}</span>
                    <span class="float-right">{{speed_stat[1]}}</span>
                </template>
            ...
            >
                ...
                small
                text
                @click="skill_stat= [27,61]"
                >Reset</v-btn>
                <v-range-slider
                :hint="`${skill_stat[0]}` +' max of ' + `${skill_stat[1]}` "
                v-model="skill_stat"
                :max="max_stat"
                :min="min_stat"
                persistent-hint
                ...
                <template v-slot:message>
                    <span class="float-left">{{skill_stat[0]}}</span>
                    <span class="float-right">{{skill_stat[1]}}</span>
                </template>
            ...
            >
                ...
                small
                text
                @click="morale_stat= [27,61]"
                >Reset</v-btn>
                <v-range-slider
                :hint="`${morale_stat[0]}` +' max of ' + `${morale_stat[1]}` "
                v-model="morale_stat"
                :max="max_stat"
                :min="min_stat"
                persistent-hint
                ...
                <template v-slot:message>
                    <span class="float-left">{{morale_stat[0]}}</span>
                    <span class="float-right">{{morale_stat[1]}}</span>
                </template>
        ...
        <v-container>
                ...
                <v-container v-if="this.$isMobile()">
                    <v-btn @click.stop="drawer = !drawer" color="primary">
                        Filter
                    </v-btn>
                </v-container>

            <h1 v-if="axie.length != 0 && axie.axies.total != 0">{{axie.axies.total}} Axies</h1>
        </v-container>
        <Axie v-if="axie.length != 0" :axielist="axie" :price="price" :loading="preloads"/>
        <Loads v-if="axie.length == 0" style="margin-top:100px" />

        <div v-if="axie.length != 0 && axie.axies.total == 0" class="d-flex justify-center mt-10">
            <v-sheet
                color="transparent"
                width="400"
                height="400"
            >
                <v-img
                    ...
                    :src="require('@/assets/empty-search.png')"
                    :lazy-src="require('@/assets/empty-search.png')"
                
                ></v-img>
                
                <h4 v-if="axie.length != 0 && axie.axies.total == 0" class="text-center grey--text body-1"> No Axie matches your filter</h4>
...
<script>
    ...
    export default {
        ...
        computed:{
            ...
            parts: function () {
                return this.selected_parts == 0 ? null : _.map(this.selected_parts, 'name')
            }
        },
        data: () => ({
            toggle_exclusive:0,
        ...
        append_selected_search_parts : function (name){

            const key = _.findIndex(this.suggestion, function(o) { return o.name == name; })

            this.selected_parts.push(this.suggestion[key])

            this.$delete(this.suggestion, key)
        },

        preppend_selected_search_parts : function (name){

            const key = _.findIndex(this.selected_parts, function(o) { return o.name == name; })

            this.suggestion.push(this.selected_parts[key])

            this.$delete(this.selected_parts, key)
        },
        ...
        load_axies: _.debounce(async function(){
            ...
            const response = await this.$axios.post('https://axieinfinity.com/graphql-server-v2/graphql',
                ...
                "variables":{
                    ...
                    "criteria":{
                        ...
                        "pureness":this.purity == 0 ? null : this.purity,
                        "title":null,
                        "breedable":null,
                        "breedCount":this.breed_count,
                        "hp":this.health_stat,
                        "skill":this.skill_stat,
                        "speed":this.speed_stat,
                        "morale":this.morale_stat
                    }
                },
          ...
          search_axie : _.debounce(async function(){
                ...
                if(store.data.axie == null){
                    ...
                    this.axie = {
                        "axies": {
                            ...
                            "results": [
                                {
                                    "id": store.data.axie.id,
                                    "name": store.data.axie.name,
                                    "image": store.data.axie.image,
                                    "class": store.data.axie.class,
                                    "breedCount": store.data.axie.breedCount,
                                    "auction": store.data.axie.auction
                                }
                            ],
                            "__typename": "Axies"
                        }
        ...
        watch:{
            ...
            morale_stat: function (){
                this.load_axies();
            },
            selected_parts : function (v){
                this.load_axies();
            },
            searchaxie : function (){
                this.search_axie()
