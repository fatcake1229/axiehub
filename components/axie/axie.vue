        ...
        <v-container class="pa-4 text-center" v-if="axielist.length != 0">
            ...
            <template v-for="(item, i) in axielist.axies.results">
                ...
                <v-hover v-slot="{ hover }" style="cursor:pointer" >
                    ...
                    style="backdrop-filter: blur(17px) saturate(132%);
                    ...
                    <v-card-title>
                        <div class="d-flex flex-column text-left caption">
                            ...
                            <div>
                                <small>Breed Count: {{item.breedCount}}</small>
                            </div>
                        </div>
                    </v-card-title>
                    ...
                    <v-card-subtitle v-if="price.length != 0 && item.auction != null">
                        ...
                        <small>
                            <!-- <s>${{item.auction.currentPriceUSD | discount}}</s> -->
                            <s>${{item.auction.currentPriceUSD}}</s>
                            ${{item.auction.currentPriceUSD | make_discount}}
                            </small>
                    </v-card-subtitle>
    ...
    export default {
        ...
        methods:{
          ...
          convert_usd_to_weth (currentprice){
              ...
              const convert = calculate / this.price.weth.usd;
              const total = parseFloat(convert).toFixed(3)
              return total
            },
